...
class Model(dict, metaclass=ModelMetaClass):
	...
	async def findNumber(cls, selectField, where=None, args=None):
		...
		sql = ['select %s _num from `%s`' % (selectField, cls.__table__)]
		if where:
			sql.append('where')
			sql.append(where)
	...
	async def update(self):
		...
		if rows != 1:
			logging.warn('failed to update by promary key: affected rows: %s' % rows)

	async def remove(self):
		args = [self.getValue(self.__primary_key__)]
		rows = await execute(self.__delete__, args)
		if rows != 1:
			logging.warn('failed to remove primary key: affected rows: %s' % rows)
